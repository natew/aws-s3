module AWS2 #:nodoc:
  # AWS2::S3 is a Ruby library for Amazon's Simple Storage Service's REST API (http://aws.amazon.com/s3).
  ...
  #   # => true
  #
  # The AWS2::S3 library ships with an interactive shell called <tt>s3sh</tt>. From within it, you have access to all the operations the library exposes from the command line.
  #
  #   % s3sh
  #   >> Version
  #
  ...
  #
  #   AWS2::S3::Base.establish_connection!(
  #     :access_key_id     => 'abc',
  ...
  #   fi
  #
  # See more connection details at AWS2::S3::Connection::Management::ClassMethods.
  module S3
    constant :DEFAULT_HOST, 's3.amazonaws.com'

    # AWS2::S3::Base is the abstract super class of all classes who make requests against S3, such as the built in
    ...
    #
    #   AWS2::S3::Base.establish_connection!(:access_key_id => '...', :secret_access_key => '...')
    #
    ...
    class Base
      class << self
        ...
        #
        #   class JukeBoxSong < AWS2::S3::S3Object
        ...
        def set_current_bucket_to(name)
          raise ArgumentError, "`#{__method__}' must be called on a subclass of #{self.name}" if self == AWS2::S3::Base
          instance_eval(<<-EVAL)
            def current_bucket
              '#{name}'
            end
